...
def _authorize_query(query: Query) -> Optional[Query]:
    ...
    base_query = query.enable_assertions(False)

    # this is to prevent recursion, we'll restore this later
    # I have no idea if this is the correct way to do this
    base_query.session = None


    # build a list of queries that we'll later union together instead of
    # combining filters, this is because sqlalchemy might generate a cross/inner
    # join because of one of the filters, which might prevent any results from
    # showing
    queries = []

    entities = {column["entity"] for column in base_query.column_descriptions}
